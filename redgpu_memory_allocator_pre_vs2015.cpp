...
public:
    ...
    ~VmaPoolAllocator();
    T* Alloc(const VmaAllocationRequest& request, VmaSuballocationType type, VkDeviceSize allocSize, VmaAllocation hAllocation);
    T* Alloc(uint32_t currentFrameIndex, bool isUserDataString);
    void Free(T* ptr);
...
private:
...
#if _MSC_VER >= 1900 // https://docs.microsoft.com/en-us/previous-versions/hh567368(v=vs.140)?redirectedfrom=MSDN#alignment
        alignas(T)
#endif
        char Value[sizeof(T)];
    };
    ...
    VmaVector<ItemBlock> m_ItemBlocks;
...
template<typename T>
T* VmaPoolAllocator<T>::Alloc(const VmaAllocationRequest& request, VmaSuballocationType type, VkDeviceSize allocSize, VmaAllocation hAllocation)
{
    for(size_t i = m_ItemBlocks.size(); i--; )
    {
        ...
        if(block.FirstFreeIndex != UINT32_MAX)
        {
            ...
            new(result)T(request, type, allocSize, hAllocation); // Explicit constructor call.
            return result;
        }
    }
...
template<typename T>
T* VmaPoolAllocator<T>::Alloc(uint32_t currentFrameIndex, bool isUserDataString)
{
    for(size_t i = m_ItemBlocks.size(); i--; )
    {
        ItemBlock& block = m_ItemBlocks[i];
        ...
        if(block.FirstFreeIndex != UINT32_MAX)
        {
            ...
            block.FirstFreeIndex = pItem->NextFreeIndex;
            T* result = (T*)&pItem->Value;
            new(result)T(currentFrameIndex, isUserDataString); // Explicit constructor call.
            return result;
        }
    }
...
#define VMA_MAP_TYPE(KeyT, ValueT) std::unordered_map<KeyT, ValueT>
...
private:
    ...
    VmaVector<VmaSuballocationList::iterator> m_FreeSuballocationsBySize;
...
private:
    ...
    */
    typedef VmaVector<VmaSuballocation> SuballocationVectorType;
...
private:
    ...
    VmaVector<VmaDeviceMemoryBlock*> m_Blocks;
    ...
    void ApplyDefragmentationMovesCpu(
        ...
        const VmaVector<VmaDefragmentationMove>& moves);
    // Saves result to pCtx->res.
    void ApplyDefragmentationMovesGpu(
        class VmaBlockVectorDefragmentationContext* pDefragCtx,
        VmaVector<VmaDefragmentationMove>& moves,
        VkCommandBuffer commandBuffer);
...
private:
    uint32_t m_Id;
    char* m_Name;
    VmaPool_T* m_PrevPool;
    VmaPool_T* m_NextPool;
...
private:
    ...
    struct BlockInfo
    {
        ...
        VmaVector<AllocationInfo> m_Allocations;
    ...
    typedef VmaVector<BlockInfo*> BlockInfoVector;
...
private:
    ...
    VmaVector<BlockInfo> m_BlockInfos;
...
public:
    VkResult res;
    bool mutexLocked;
    VmaVector<VmaBlockDefragmentationContext> blockContexts;
    VmaVector<VmaDefragmentationMove> defragmentationMoves;
...
private:
    ...
    VmaVector<AllocInfo> m_Allocations;
    bool m_AllAllocations;
};
...
private:
    ...
    // Owner of these objects.
    VmaVector<VmaBlockVectorDefragmentationContext*> m_CustomPoolContexts;
};
...
public:
    ...
    VmaAllocation Allocate(uint32_t currentFrameIndex, bool isUserDataString);
...
{
public:
    VmaStringBuilder(VmaAllocator alloc) : m_Data() { }
    ...
    const VmaVector<VmaDefragmentationMove>& moves)
{
    ...
    VmaVector<BlockInfo>
        blockInfo(blockCount, BlockInfo());
...
VmaAllocation VmaAllocationObjectAllocator::Allocate(uint32_t currentFrameIndex, bool isUserDataString)
{
    VmaMutexLock mutexLock(m_Mutex);
    return m_Allocator.Alloc(currentFrameIndex, isUserDataString);
}
